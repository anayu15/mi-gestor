...
interface EditarDatosFacturacionModalProps {
  isOpen: boolean;
...
export default function EditarDatosFacturacionModal({ isOpen, configId, onClose, onSuccess }: EditarDatosFacturacionModalProps) {
  ...
  // Load config data when modal opens
  useEffect(() => {
    if (isOpen && configId) {
      loadConfig();
    }
  ...
  async function loadConfig() {
    ...
    try {
      const response = await billingConfigs.get(configId);
      const config = response.data;

      // Format IBAN with spaces for display
      let formattedIban = config.iban || '';
      if (formattedIban) {
        formattedIban = formattedIban.match(/.{1,4}/g)?.join(' ') || formattedIban;
      }
      ...
      setFormData({
        ...
        iban: formattedIban,
        notas_factura: config.notas_factura || '',
  ...
  async function handleLogoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const response = await billingConfigs.uploadLogo(configId, file);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // Don't send logo_url in update - it's handled separately
      const { logo_url, ...updateData } = formData;
      await billingConfigs.update(configId, updateData);
  ...
  const logoPreviewUrl = formData.logo_url
    ? `${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '')}/uploads/${formData.logo_url}`
    : null;
    ...
    <>
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden border border-gray-200">
        ...
        <div className="flex justify-between items-center px-5 py-3.5 border-b bg-gradient-to-r from-slate-50 to-gray-50">
          <h2 className="text-lg font-bold text-slate-800">Editar Configuración de Facturación</h2>
          <button
            onClick={handleClose}
          ...
          {loading ? (
            <div className="flex items-center justify-center py-8">
              <div className="text-center">
                ...
                <p className="mt-4 text-slate-600 text-sm">Cargando configuración...</p>
              </div>
            </div>
          ) : (
            <>
                ...
                <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-16 h-16 rounded-lg flex items-center justify-center overflow-hidden">
                      {logoPreviewUrl ? (
                        <img src={logoPreviewUrl} alt="Logo" className="w-full h-full object-contain bg-white border border-gray-300 rounded-lg" />
                    ...
                    <div>
                      ...
                      <label
                        htmlFor="logo-upload"
                        className={`inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md cursor-pointer transition-colors ${
                          uploadingLogo
                      ...
                      <p className="text-xs text-gray-500 mt-1">JPG o PNG, máx 2MB</p>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">
                          {formData.activo
                            ? 'Esta configuración está disponible para generar facturas'
                            : 'Esta configuración no está disponible para generar facturas'}
                        </p>
                      </div>
                      <button
                        type="button"
