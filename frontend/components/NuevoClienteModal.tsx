...
export default function NuevoClienteModal({ isOpen, onClose, onSuccess }: NuevoClienteModalProps) {
  ...
  // Check for principal client when modal opens
  useEffect(() => {
    if (isOpen) {
  ...
  async function checkPrincipalClient() {
    try {
      ...
      const principalClient = response.data.find((c: any) => c.es_cliente_principal);
                ...
                <div className="md:col-span-2">
                  <label className={`flex items-center p-3 rounded-lg border ${
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      disabled={hasPrincipalClient}
                      className="mr-3 h-4 w-4 disabled:cursor-not-allowed"
                    />
                    <div className="flex-1">
                      <span className={`font-medium text-sm ${
                        hasPrincipalClient ? 'text-gray-500' : 'text-gray-900'
                      }`}>
