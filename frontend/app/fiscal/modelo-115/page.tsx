...
export default function Modelo115Page() {
  ...
  const { isAllowed, loading: accessLoading, refresh: refreshAccess } = useModelAccess('115');
  const currentYear = new Date().getFullYear();
  const currentQuarter = Math.ceil((new Date().getMonth() + 1) / 3);

  const [year, setYear] = useState(currentYear);
  const [trimestre, setTrimestre] = useState(currentQuarter);
  const [data, setData] = useState<Modelo115Data | null>(null);
  ...
  async function loadModelo115() {
    ...
    try {
      ...
      const response = await tax.modelo115(year, trimestre);
      ...
      if (err.message.includes('Token') || err.message.includes('autenticación')) {
        ...
        setError(err.message || 'Error al cargar el Modelo 115');
      }
    } finally {
      setLoading(false);
    }
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              Modelo 115 no habilitado
            </h2>
            <p className="text-gray-700 mb-6">
              Has desactivado el Modelo 115 en tu configuracion.
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {!showSettings && loading && (
          <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-12 text-center mb-6">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto"></div>
            <p className="mt-4 text-gray-600">Cargando datos del Modelo 115...</p>
          </div>
        )}
            ...
            <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6 mb-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg border border-gray-100">
                  <span className="text-gray-700">Base de alquileres:</span>
                  <span className="text-xl font-bold text-gray-900">{formatEuro(data.base_alquiler)}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg border border-gray-100">
                  <span className="text-gray-700">Retencion 19%:</span>
                  <span className="text-xl font-bold text-orange-600">{formatEuro(data.retencion_19pct)}</span>
                </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-right">
                      <div className={`text-2xl font-bold ${
                        data.accion === 'A INGRESAR' ? 'text-orange-600' :
                        data.accion === 'SIN ACTIVIDAD' ? 'text-gray-500' : 'text-green-600'
                      }`}>
                        {formatEuro(data.resultado)}
                      </div>
            ...
            {data.nota && (
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
                <div className="flex gap-2">
                  ...
                  <p className="text-sm text-amber-900">{data.nota}</p>
                </div>
              </div>
            )}
