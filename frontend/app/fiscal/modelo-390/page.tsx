...
export default function Modelo390Page() {
  ...
  const { isAllowed, loading: accessLoading, refresh: refreshAccess } = useModelAccess('390');
  const currentYear = new Date().getFullYear();

  const [year, setYear] = useState(currentYear - 1);
  const [data, setData] = useState<Modelo390Data | null>(null);
  ...
  async function loadModelo390() {
    ...
    try {
      ...
      const response = await tax.modelo390(year);
      ...
      if (err.message.includes('Token') || err.message.includes('autenticación')) {
        ...
        setError(err.message || 'Error al cargar el Modelo 390');
      }
    } finally {
      setLoading(false);
    }
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              Modelo 390 no habilitado
            </h2>
            <p className="text-gray-700 mb-6">
              Has desactivado el Modelo 390 en tu configuracion.
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex justify-between items-center px-5 py-3">
            <div className="flex items-center gap-3">
              ...
              <div className="text-sm text-gray-500">
                <p className="text-xs">Plazo: hasta el 30 de enero de {year + 1}</p>
              </div>
            </div>
        ...
        {!showSettings && loading && (
          <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-12 text-center mb-6">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto"></div>
            <p className="mt-4 text-gray-600">Cargando datos del Modelo 390...</p>
          </div>
        )}
            ...
            <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6 mb-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg border border-gray-100">
                  <span className="text-gray-700">Base imponible total:</span>
                  <span className="text-xl font-bold text-gray-900">{formatEuro(data.resumen_anual.base_imponible_total)}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg border border-gray-100">
                  <span className="text-gray-700">IVA Repercutido (cobrado):</span>
                  <span className="text-xl font-bold text-green-600">{formatEuro(data.resumen_anual.iva_repercutido_total)}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg border border-gray-100">
                  <span className="text-gray-700">IVA Soportado (deducible):</span>
                  <span className="text-xl font-bold text-red-600">{formatEuro(data.resumen_anual.iva_soportado_total)}</span>
                </div>
                <div className="border-t-2 border-slate-200 pt-4 mt-4">
                  <div className="flex justify-between items-center">
                    <span className="text-lg font-semibold text-gray-900">RESULTADO ANUAL:</span>
                    <div className="text-right">
                      <div className={`text-2xl font-bold ${
                        data.resumen_anual.resultado_total > 0 ? 'text-blue-600' :
                        data.resumen_anual.resultado_total < 0 ? 'text-green-600' : 'text-gray-500'
                      }`}>
                        {formatEuro(Math.abs(data.resumen_anual.resultado_total))}
                      </div>
                      <div className="text-sm text-gray-600">{data.accion}</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6 mb-6">
                  ...
                  <div key={trimestre.trimestre} className="p-4 bg-gray-50 rounded-lg border border-gray-100">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-green-600">{formatEuro(trimestre.iva_repercutido)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">IVA Soportado:</span>
                        <span className="font-medium text-red-600">{formatEuro(trimestre.iva_soportado)}</span>
                      </div>
                      <div className="flex justify-between pt-2 border-t border-gray-200">
                        <span className="text-gray-900 font-medium">Resultado:</span>
                        <span className={`font-bold ${
                          trimestre.resultado > 0 ? 'text-blue-600' :
                          trimestre.resultado < 0 ? 'text-green-600' : 'text-gray-500'
                        }`}>
                          {formatEuro(Math.abs(trimestre.resultado))}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="mt-4 p-3 bg-slate-50 border border-slate-200 rounded-lg">
                <p className="text-sm text-slate-700">
                  <strong>Verificacion:</strong> La suma de los 4 trimestres debe coincidir con el total anual.
                  Total calculado: <span className="font-bold">
                    {formatEuro(data.desglose_trimestral.reduce((sum, t) => sum + t.resultado, 0))}
                  </span>
                </p>
              </div>
            </div>
